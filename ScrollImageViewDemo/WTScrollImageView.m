...
@interface WTScrollImageView()<UIScrollViewDelegate>
//轮播的图片数组
...
@implementation WTScrollImageView
#pragma mark- 初始化方法
//创建用来缓存图片的文件夹
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"WTScrollImageView"];
    BOOL isDir = NO;
...
+ (instancetype)carouselViewWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray {
    WTScrollImageView *carouselView = [[self alloc] init];
...
- (void)downloadImages:(int)index {
    ...
    //从沙盒中取图片
    NSString *path = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"WTScrollImageView"] stringByAppendingPathComponent:[key lastPathComponent]];
