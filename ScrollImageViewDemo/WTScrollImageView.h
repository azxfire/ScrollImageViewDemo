...
@protocol WTScrollImageViewDelegate <NSObject>
 ...
 *  @param index        图片索引
 */
- (void)carouselView:(WTScrollImageView *)carouselView clickImageAtIndex:(NSInteger)index;
 ...
 *  其他属性都有默认值，可不设置
 */
@interface WTScrollImageView : UIView
 ...
 *  代理，用来处理图片的点击
 */
@property (nonatomic, weak) id<WTScrollImageViewDelegate> delegate;
